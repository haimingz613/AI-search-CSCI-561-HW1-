...
public class A {
    ...
    public A(Year_Location initial, Year_Location target, Map<Year_Location, List<Year_Location>> graph, int width, int height){
        queue = new PriorityQueue<>(new Comparator<List<Year_Location>>() {
            @Override
            public int compare(List<Year_Location> o1, List<Year_Location> o2) {
                int used = o1.get(o1.size() - 1).total_cost - o2.get(o2.size() - 1).total_cost;
                int future1 = Math.abs(o1.get(o1.size() - 1).x - target.x) + Math.abs(o1.get(o1.size() - 1).y - target.y) + Math.abs(o1.get(o1.size() - 1).year - target.year);
                int future2 = Math.abs(o2.get(o2.size() - 1).x - target.x) + Math.abs(o2.get(o2.size() - 1).y - target.y) + Math.abs(o2.get(o2.size() - 1).year - target.year);
                return used + future1 - future2;
            }
        });
        visited = new HashSet<>();
        time = 0;
